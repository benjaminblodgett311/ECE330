## Benjamin Blodgett & Erik Garringer / Group 5 / Lab 2

# Part 1
LD5 is blinking red, but hitting the reset button makes all the user leds flash.

# Part 2

## a / b

LD3 is blinking orange, but hitting the reset button makes all the user leds flash.

## c
LD4 is blinking green, but hitting the reset button makes all the user leds flash.

## linux commands
```bash
$ vim carrots.c
$ for f in carrots.c flour.c; do
arm-none-eabi-gcc -c -mthumb -mcpu=cortex-m4 -o "$(basename "$f" .c).o" $f;
done
$ arm-none-eabi-ld -T recipe.ld -o cakemix.elf carrots.o flour.o sugar.o
$ arm-none-eabi-objcopy -O binary cakemix.elf cakemix.bin
```
...
> reset halt
> flash write_image erase cakemix/cakemix.bin 0x08000000
> reset
```
